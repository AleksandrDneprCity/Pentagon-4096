...
2) Формирование видеосигнала с полностью стандартными синхро-импульсами и импульсов гашения, которые гарантируют нормальную работу с любым 
...
9) Слота ZX-Bus Plus-3 не будет. Вместо него будет специальная райзер плата-адаптер +3, в которую можно будет устанавливать карты расширения 3+.
Дополнительной задачей райзер платы +3 будет разрешение конфликтов устройства DivIDE с GeneralSound, у которых совпадают 2 порта 0xB3 и 0xBB.

10) Плюс 1 слот ISA специально для модема, включенного по схеме Кондратьева, полный вариант с прерыванием от модема IRQ4 по NMI. Использовать 
...
18) Теневое ОЗУ ёмкостью 128 Кб. Будет выполнять 2 функции - в качестве стандартного теневого ОЗУ, включающееся и выключающееся через 
...
20.7) "FlashColor". Можно применять в комбинации с видеорежимами 20.0, 20.1, 20.3, 20.4 и 20.6. Внутри каждого цветного атрибута, в котором активирован 7-й бит "флеш", вместо стандартного мигация знакоместа, перемешиваются цвета пикселей и фона и выводятся результат для пикселей, цвет фона при этом чёрный.
...
20.9) Подумываю над реализацией регистра палитры для возможности переопределять стандартные 16 цветов на любые произвольные.
...
EM0      EM1          SIMM 1M          SIMM 4M            Max Size
0  0     0  0         128K             128K                128K
0  1     0  0         Pent-1M          Pent-1M              1M (#7FFD биты 6, 7, 5)
1  0     0  0         Profi-1M         Profi-1M             1M (#DFFD биты 0, 1, 2)
1  1     0  0         #1FFD            #1FFD               512K (#1FFD биты 4, 7)
1  1     0  1         KAY-1M           KAY-1M+Pent          4M (#1FFD биты 4, 7, #7FFD биты 6, 7, 5)
1  0     1  0         Profi-1M         Profi-1M+Pent        4M (#DFFD биты 0, 1, 2, #7FFD биты 6, 7)
1  0     1  1         Profi-1M         Profi-1M+1FFD        4M (#DFFD биты 0, 1, 2, #1FFD биты 4, 7)
