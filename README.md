...
Описание портов BIOS:

#FC37 "Конфигурация памяти":

    bit 0, 1 - выбор 1-го порта расширения ОЗУ > 128 КБ (EM0):
    0 - выкл,
    1 - Пентагон-1024 (#7FFD)
    2 - Профи-1024 (#DFFD)
    3 - Scorpion/KAY (#1FFD)

    bit 2, 3 - выбор 2-го порта расширения ОЗУ > 128 КБ (EM1):
    0 - выкл,
    1 - Пентагон-1024 (#7FFD)
    2 - Профи-1024 (#DFFD)
    3 - Scorpion/KAY (#1FFD)

Совместное использование 1-го и 2-го портов расширения (EM0 и EM1) подчиняется следующей таблице:

        Биты порта #FC37:
        3  2     1  0
        EM1      EM0          SIMM 1M          SIMM 4M            Max Size                                      Size (без участия бита защёлки D5 7FFD)
    
    0)  0  0     0  0         128K             128K                128K                                          128K
    1)  0  0     0  1         Pent-1M          Pent-1M              1M (#7FFD биты 6, 7, 5)                      512K (#7FFD биты 6, 7)
    2)  0  0     1  0         Profi-1M         Profi-1M             1M (#DFFD биты 0, 1, 2)                       1M (#DFFD биты 0, 1, 2)
    3)  0  0     1  1         #1FFD            #1FFD               512K (#1FFD биты 4, 7)                        512K (#1FFD биты 4, 7)
    4)  0  1     1  1         KAY-1M           KAY-1M+Pent          4M (#1FFD биты 4, 7, #7FFD биты 6, 7, 5)      2M (#1FFD биты 4, 7, #7FFD биты 6, 7)
    5)  1  0     1  0         Profi-1M         Profi-1M+Pent        4M (#DFFD биты 0, 1, 2, #7FFD биты 6, 7)      4M (#DFFD биты 0, 1, 2, #7FFD биты 6, 7)
    6)  1  1     1  0         Profi-1M         Profi-1M+1FFD        4M (#DFFD биты 0, 1, 2, #1FFD биты 4, 7)      4M (#DFFD биты 0, 1, 2, #1FFD биты 4, 7)

Примечание-1: В любой из выбранной конфигурации ОЗУ, где задействуется бит 5 порта #7FFD для расширения ОЗУ, доступность такого расширения зависит от опции BIOS "Бит 5 #7FFD защёлка или расширение".

Примечание-2: Конфигурация № 5 является исключением из правил - для задействования двух портов #DFFD и #7FFD, EM0 и EM1 оба должны быть = 2.

    bit 4 - опция "Бит 5 #7FFD защёлка или расширение". Если этот бит = 0, тогда бит 5 порта #7FFD расширяет ОЗУ, в противном случае работает как обычная защёлка 48 КБ.
    bit 5 - включение режима загрузки данных в статическое ОЗУ. При активации этого бита, в адреса #C000 - #FFFF вместо основного ОЗУ подставляется статическое ОЗУ с полным доступом. 
Выбор страницы статического ОЗУ осуществляется через порт #7FFD битами 0 и 1, что даёт доступ в пределах 64 КБ, а так же включение или отключением режима "3+", что даёт выбор между первыми и вторыми 64 КБ статического ОЗУ. Если в порту #7FFD активировать бит 2, то вместо статического ОЗУ снова включается основное ОЗУ. Сделал это для возможности использовать отладчик STS 3.3, который висит в 7-й странице ОЗУ.
    bit 6 - включение режима "Мультизадачность".
    bit 7 - включение режима "3+".

#FD37: - выбор страниц эмуляции основного ПЗУ в статическом ОЗУ. При активации какого-либо бита этого порта, выбранная страница основного ПЗУ, которая соответствует биту порта BIOS, подменяется на соответствующую страницу из статического ОЗУ. В этом режиме выбранные страницы статического ОЗУ доступны в режим "только чтение".

#FE37: - дополнительные видеорежимы.

    bit 0, 1, 2, 3 - описывает цвет текста для монохромного режима 512х192. B, R, G и яркость.
    bit 4 - включение цвета для режима 512х192.
    bit 5 - включение режима "Флеш-колор".
    bit 6 - включение авторежима "Гигаскрин".
    bit 7 - выбор положения сигнала INT. 0 = "Pentagon"? 1 = "Фирменный".

#FF37: - управление ПЗУ и блокировками.

    bit 0 - отключает ПЗУ BIOS и включает основное ПЗУ "Spectrum".
    bit 1 - отключает блокировку защёлки сервисной страницы ПЗУ "Gluk Reset Service".
    bit 2, 3 - выбор страницы ПЗУ BIOS в пределах первых 3-х страниц. Последняя страница ПЗУ BIOS задействована в качестве ПЗУ Принтера, поэтому недоступна для режима BIOS.
    bit 4 - блокировка включения режима "кеш память".
    bit 5 - разрешает запись в регистры палитры.
    bit 6 - включение режима записи в основное ПЗУ "Spectrum".
    bit 7 - включение режима стирания основного ПЗУ "Spectrum".

Раскладка порта конфигурации #EFF7:

    bit 0 - видеорежим "16 Colors" - каждый пиксель своим цветом.
    bit 1 - видеорежим "512х192" - удвоенное разрешение по горизонтали.
    bit 2 - не используется.
    bit 3 - проецирование 0-й страницы ОЗУ в адресное пространство ПЗУ #0000 - #3FFF.
    bit 4 - не используется.
    bit 5 - видеорежим "Мультиколор" - атрибут цвета на байт.
    bit 6 - видеорежим "384x304" - экран без бордюра.
    bit 7 - открытие скрытых портов RTC и FDC.
    
Раскладка порта конфигурации #FE37:

    bit 0, 1, 2 - цвет текста (B, R, G) в монохромном режиме 512x192.
    bit 3 - яркость текста в монохромном режиме 512x192.
    bit 4 - включение цвета в режиме 512x192.
    bit 5 - видеорежим "FlashColor".
    bit 6 - видеорежим "Аппаратный GigaScreen с автовключением".
    bit 7 - выбор положения INT: 0 = Пентагон, 1 = Фирменный.
    
Видеорежимами можно управлять двумя портами: 

#EFF7 - основной стандартный порт управления видеорежимами;

#FE37 - дополнительный порт BIOS управления видеорежимами.

        #EFF7:                  #FE37:
    0)  X 0 0 X - X X 0 0       X 0 0 X - X X X X        Стандартный видеорежим
    1)  X 0 1 X - X X 0 0       X X X X - X X X X        "Аппаратный мультиколор" (атрибут цвета на байт)
    2)  X 0 0 X - X X 0 1       X X 0 X - X X X X        "16 Colors" (каждый пиксель своим цветом)
    3)  X 0 0 X - X X 1 0       X X X 0 - I G R B        "512x192 монохромный", цвет текста задаётся битами I G R B порта #FE37
    4)  X 0 0 X - X X 1 0       X X X 1 - X X X X        "512x192 цветной", атрибут цвета на узкое знакоместо размером 8х8 пикселей
    5)  X 0 1 X - X X 1 0       X X X 1 - X X X X        "512x192 цветной мультиколорный" - атрибут цвета на узкий байт размером 8x1 пикселей
    6)  X 1 0 X - X X 0 0       X X X X - X X X X        "384x304" - экран без бордюра, количество пикселей без учёта строчных и кадровых гасящих импульсов
    7)  X X X X - X X X 0       X X 1 X - X X X X        "FlashColor" - бит 7 атрибута цвета вместо мигания активирует палитру INK (кроме монохромного режима 512х192 и 16 Colors)
    8)  X X X X - X X X X       X 1 X X - X X X X        "Аппаратный GigaScreen" - перекл. осн. и доп. экран каждую строку, вкл/выкл автоматически по факту программного переключения экранов

Примечание-1:
При включении через порт #FE37 режима "Аппаратный GigaScreen", фактически он включается с момента однократного программного переключения экранов, осуществляя их аппаратное переключение вначале каждой строки, и отключается в конце кадра.

Примечание-2:
Видеорежим "16 Colors" не совместим с видеорежимами "Мультиколор", "512x192" цветным и монохромным, "384x304" и "FlashColor".

Примечание-3:
Видеорежим "384x304" не совместим с видеорежимами "Мультиколор", "512x192" цветным и монохромным и "16 Colors".

Примечание-4:
Видеорежим "Аппаратный GigaScreen" совместим со всеми остальными видеорежимами, его можно комбинировать с любым другим видеорежимом.
